...
| Couplings-aware in-frame aligner (gap-placement min) | `src/SBM/energy/potts_align.py` — `enumerate_align` (exact global Potts-frame argmin for `N≤L` queries; few-gap), `pt_align` (parallel tempering w/ teleport move, the g-adaptive default high-gap engine), `sa_align`, `potts_align` (dispatch: enum if `C(L,N)≤budget` else g-adaptive PT), `PTSchedule.for_gap_count`, `perturb_frame`. The iter-003 §10.20 fix for the DCAlign worse-than-native residual (a BP search failure). Full replicable spec + cost model + DCAlign comparison: `docs/POTTS_ALIGN.md`. Driver `scripts/analyze_potts_align.py`; tests `tests/test_potts_align.py`. Handles cross-family when `N≤L`; **not** `N>L` (insertions). |
| iter-003 combine wiring (evaluate `potts_align` as the aligner, no DCAlign) | `docs/ITER003_RUNBOOK.md` — exact spec for scoring every (query,model) pair with `N≤L` via `potts_align`, sharded over a Slurm array (pure Python, no Julia). Cost capped at **~100 SU** by subsampling the PPIC→CM cross block (the 241-SU driver) to 8000 seeded + adding 500 random N=91 controls (negative control for the `two_model_energy` figure). |
...
| The DCAlign diagnostics (Blocker-1 baseline + convergence) | pure logic `src/SBM/energy/dcalign_baseline.py` (`column_agreement`/`compare_record`/`summarize` for the baseline; `convergence_by_group`/`summarize_convergence` for convergence) + figures `src/SBM/utils/utils_dcalign_baseline_plot.py` & `utils_dcalign_convergence_plot.py` + CLIs `scripts/compare_dcalign_baseline.py` & `report_dcalign_convergence.py` + wrappers `scripts/wf/run_dcalign_{baseline,convergence}.py`; the `dcalign_baseline` + `dcalign_convergence` rules in `Snakefile.combine`. Baseline compares DCAlign's energy to the native in-frame energy per home-pair sequence (`ΔE>0` ⇒ DCAlign worse); convergence counts decimation-fallback per (model, group) over all alignments. See `docs/initiate_two_model_energy.md` §10.12 |
...
- **Rules:** `snapshot_config` → `resolve_models` (`models.json`: paths, sha256s, L, seed-MSA per model) → `build_query` (`query/query.fasta` + `query/groups.json`) → `score` (`data/scores.tsv` tidy long-form + `data/scores_detail.json` + `provenance/score_manifest.json`) → `render_combine` (`figs/two_model_energy.pdf`, one consolidated `E_A` vs `E_B` scatter + marginals) → `run_manifest` (`provenance/run_manifest.json`). When `scoring.method == "dcalign"`, two extra rules run: `dcalign_baseline` (Blocker-1 diagnostic — `data/dcalign_vs_inframe.{tsv,json}` + `figs/dcalign_vs_inframe.pdf`, DCAlign energy vs the native in-frame energy per home-pair sequence) and `dcalign_convergence` (`data/dcalign_convergence.{tsv,json}` + `figs/dcalign_convergence.pdf`, non-convergence counts per (model, group) over all alignments) (§10.12).
- **Run-dir layout:** Mac-side outputs are grouped into `data/` (tables) and `provenance/` (manifests); `figs/` holds figures. The top level keeps only the **Mac↔Midway contract** files the cluster DCAlign scripts read by exact path — `config_snapshot.yaml`, `models.json`, `query/` — plus `iteration_note.md`, `dcalign/`, `logs/`. `sync_models.sh` selects by directory-name prune, so `data/`/`provenance/` sync automatically.
- **Methods** (`scoring.method`): `map` (**default** for combine; Viterbi/fields-MAP — the single best alignment per model, same procedure for both → comparable `E_A`/`E_B`), `marginal` (importance-sampling free energy, spec §3.1 — the only mode that yields ESS + MC stderr; warns when ESS < threshold), `in_frame` (exact Potts sum, spec §2), and `auto` (in-frame via the original MSA alignment for a sequence's home model, marginal for the other — **breaks A/B comparability, so it warns**), and `dcalign` (couplings-aware alignment by DCAlign, spec §10.9; the upgrade over fields-MAP). `map` is *fields*-MAP (ignores couplings when choosing the alignment); `dcalign` uses the full couplings. Because DCAlign is ~700× slower (median 19 s/seq), the expensive Julia alignment runs **out-of-process on the cluster** (`pipeline/external/`, sbatch array) and is **cached on disk** under `combine/<run>/dcalign/cache/<model>/alignments.tsv`; the `dcalign` score branch is then a thin cache-reader that recomputes the energy in-frame via `potts_energy` (gauge-consistent — the in-frame recompute vs DCAlign's own energy agrees ≤5e-7, a standing manifest canary). So `score_sequence` never shells out to Julia. The insertion prior is set by `scoring.lambda_spec`: `"flat"` (geometry-blind, mass on Δn=1 — the iter-001 baseline that mis-aligned natives, spec §10.8/§10.12 Blocker 1) or `"deltan"` (the informed `DCAlign.deltan_prior` built from each model's own seed MSA, in the exact L-frame — spec §10.13; `align_sequences` stages it as `seed.ins`). The HMM proposal is a **self-contained numpy profile HMM** (`src/SBM/energy/hmm.py`) with match emissions from `h`, validated against brute force — no HMMER/pyhmmer dependency.
