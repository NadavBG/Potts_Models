...
def _autodiscover_synthetic(run_dir: Path) -> list[Path]:
    ...
    ``rglob``) so any subdirectory artifacts don't leak into the energy
    / similarity / diversity / pca / correlations figures.
    """
...
def _render_one(
    ...
    *,
    ...
    Figures are dispatched to ``plot_stats(plot=mode, artificial=...)``,
    which draws into figures it creates internally; we diff
    ``plt.get_fignums()`` to capture them.

    ``artificial`` is the per-alignment list of dicts (with
    ``temperature``, ``align_mod``, ``stats``, ``color``); ignored by
    Coupling_evol and Params. ``natural_colors`` carries Train/Test/Random
    colors. ``sector_positions`` is read only by the Params figure.
